...
public class CustomDynamicInterceptor extends BaseDynamicInterceptor<CustomDynamicInterceptor> {
    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        if (request.body() instanceof RequestBody) {
            ...
            try {
                String requestParams = "{\"requestData\":" + "\"" + AesEncryptUtils.encrypt(params).trim() + "\"}";
                Log.e("XHttp===>加密参数:", requestParams.trim());
                RequestBody requestBody = RequestBody.create(MediaType.parse("application/json;charset=UTF-8"), requestParams.trim());
                String token = UserProfile.getInstance().getAppToken();
                if (isAccessToken() && !TextUtils.isEmpty(token)) {
                    return chain.proceed(request.newBuilder().addHeader("token", token).post(requestBody).build());
                } else {
