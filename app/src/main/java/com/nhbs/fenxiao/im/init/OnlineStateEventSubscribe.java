...
public class OnlineStateEventSubscribe {
    ...
    public static void initSubscribes() {
        ...
        if (timeInterval <= SUBS_FREQ) {
            ...
            Handler handler = Handlers.sharedHandler(NimAppCache.getContext());
    ...
    public static void subscribeOnlineStateEvent(final List<String> accounts, long expiry) {
        ...
        updateLastSubsTime();
        NIMClient.getService(
            EventSubscribeService.class).subscribeEvent(eventSubscribeRequest).setCallback(new RequestCallbackWrapper<List<String>>() {
            @Override
