...
public class OnlineStateEventManager {
    ...
    private static ContactChangedObserver observer = new ContactChangedObserver() {
        ...
        public void onDeletedFriends(final List<String> accounts) {
            ...
            NIMClient.getService(
                MsgService.class).queryRecentContacts().setCallback(new RequestCallbackWrapper<List<RecentContact>>() {
                @Override
    ...
    private static void registerOnlineStatusObserver() {
        NIMClient.getService(
            AuthServiceObserver.class).observeOnlineStatus(new Observer<StatusCode>() {
            @Override
    ...
    private static void registerNetTypeChangeObserver() {
        ...
        NimAppCache.getContext().registerReceiver(receiver, filter);
    }
    ...
    private static void registerEventObserver(boolean register) {
        NIMClient.getService(
            EventSubscribeServiceObserver.class).observeEventChanged(new Observer<List<Event>>() {
            @Override
    ...
    public static void publishOnlineStateEvent(boolean force) {
        ...
        int netState = getNetWorkTypeName(NimAppCache.getContext());
    ...
    private static boolean enableOnlineStateEvent() {
        String packageName = NimAppCache.getContext().getPackageName();
