...
public class MineWithdrawActivity extends BaseBarActivity implements MineWithdrawViewer {
    ...
    protected void loadData() {
        ...
        bindView(R.id.tv_withdraw, view -> {
            if (TextUtils.isEmpty(tv_account.getText().toString().trim())) {
...
//        bundle.putString("");
    ...
    public void getUserInfoSuccess(MineUserInfoBean mineUserInfoBean) {
        if (mineUserInfoBean != null) {
            if (withdraw_type == 1) {
                //支付宝
                bindText(R.id.tv_account, mineUserInfoBean.aliAccount);
            } else {
                //微信
                bindText(R.id.tv_account, mineUserInfoBean.winXinName);
            }
