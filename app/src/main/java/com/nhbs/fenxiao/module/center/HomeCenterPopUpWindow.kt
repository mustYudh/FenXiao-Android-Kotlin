...
class HomeCenterPopUpWindow(context: Activity) : BasePopupWindow(
  ...
  private fun dismiss(call: (() -> Unit)?) {
    ...
    val array = floatArrayOf(DensityUtils.dp2px(context, 45F).toFloat(),
        DensityUtils.dp2px(context, 0F).toFloat())
    showAnimator(cancelView!!, "rotation", 200, array, null) {
      ...
      StatusBarFontColorUtil.statusBarDarkMode(activity!!)
      val array1 =
          floatArrayOf(DensityUtils.dp2px(context, -220F).toFloat(),
              DensityUtils.dp2px(context, 88F).toFloat())
      val array2 =
          floatArrayOf(DensityUtils.dp2px(context, -220F).toFloat(),
              DensityUtils.dp2px(context, 88F).toFloat())
      val array3 =
  ...
  override fun showPopupWindow() {
    val array = floatArrayOf(DensityUtils.dp2px(context, 0F).toFloat(),
        DensityUtils.dp2px(context, 45F).toFloat())
    StatusBarFontColorUtil.statusBarDarkMode(activity!!)
    val array1 =
        floatArrayOf(DensityUtils.dp2px(context, -88F).toFloat(),
            DensityUtils.dp2px(context, -220F).toFloat())
    val array2 =
        floatArrayOf(DensityUtils.dp2px(context, -88F).toFloat(),
            DensityUtils.dp2px(context, -220F).toFloat())
    val array3 =
  ...
  override fun onClick(v: View?) {
    when (v?.id) {
      R.id.release_goods -> {
        ...
        dismiss {
          if (UserProfile.getInstance().isMerchant == 1) {
            LauncherHelper.from(activity).startActivity(ReleaseGoodsActivity::class.java)
          } else {
            showToast("请先开通店铺")
          }
        }
      ...
      R.id.advertising -> {
        dismiss {
          if (UserProfile.getInstance().isMerchant == 1) {
            LauncherHelper.from(activity).startActivity(ReleaseAdvertisingActivity::class.java)
          } else {
            showToast("请先开通店铺")
          }
      ...
      R.id.activity -> {
        dismiss {
          if (UserProfile.getInstance().isMerchant == 1) {
            LauncherHelper.from(activity).startActivity(ReleaseACGoodsActivity::class.java)
          } else {
            showToast("请先开通店铺")
          }
